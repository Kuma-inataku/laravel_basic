...
final class AddPointUseCase
{
     ...
     *
     */
    public function run(int $customerId, int $addPoint, string $pointEvent, CarbonImmutable $now): int
    {
        if ($addPoint <= 0) {
        ...
        if (! $this->eloquentCustomerPoint->where('customer_id', $customerId)->exists()) {
            $message = sprintf('customer_points.customer_id:%d does not exists', $customerId);
            throw new PreconditionException($message);
        }
